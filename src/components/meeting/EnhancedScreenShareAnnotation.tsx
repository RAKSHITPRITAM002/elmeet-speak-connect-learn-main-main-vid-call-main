...
import ZoomStyleAnnotationToolbar from './ZoomStyleAnnotationToolbar';
import { AnnotationCanvasRef } from './AnnotationCanvas';

interface EnhancedScreenShareAnnotationProps {
  isScreenSharing: boolean;
  screenShareRef: React.RefObject<HTMLVideoElement>;
  onToggleAnnotation?: (isAnnotating: boolean) => void;
}

const EnhancedScreenShareAnnotation: React.FC<EnhancedScreenShareAnnotationProps> = ({
  isScreenSharing,
  screenShareRef,
  onToggleAnnotation
}) => {
  ...
  }, [isScreenSharing, screenShareRef, currentColor, currentWidth]);
  ...
  const redrawAnnotations = () => {
    ...
    annotations.forEach(annotation => {
      ...
      if (annotation.tool === 'pen' || annotation.tool === 'eraser') {
        ...
        drawRectangle(annotation.x, annotation.y, annotation.w, annotation.h);
  ...
  const undoAnnotation = () => {
    ...
    const lastAnnotation = newAnnotations.pop();
  ...
  const redoAnnotation = () => {
    ...
    const annotationToRedo = newRedoStack.pop();
    
    if (annotationToRedo) {
      // Add back to annotations
      setAnnotations([...annotations, annotationToRedo]);
    ...
    <div ref={containerRef} className="relative w-full h-full">
      ...
      {isScreenSharing && !isAnnotating && (
        <Button
          ...
          className="absolute top-4 right-4 bg-blue-600 hover:bg-blue-700 text-white z-20"
        >
          Annotate
        </Button>
      )}
      ...
      {isScreenSharing && (
        <canvas
          ref={canvasRef}
          className={`absolute top-0 left-0 w-full h-full ${isAnnotating ? 'z-10' : 'z-0 pointer-events-none'}`}
...
export default EnhancedScreenShareAnnotation;